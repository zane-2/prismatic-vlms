...
echo "Run fig_2_2_seed2"

torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  --model.type "one-stage+7b" \
  --model.model_id "prism-clip+7b-webvid-train-20k-total-input-frames=16-videos-per-cluster=2-epochs=1-gpus=2-seed2" \
  --dataset.type "webvid" \
  ...
  --dataset.finetune_stage_components '["/vision/u/zanedurante/prismatic-vlms/dataset_splits/webvid_160k_video_split.json_video_20000_frames_8.json", ""]' \
