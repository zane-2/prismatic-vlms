...
echo "Run fig_2_8_seed1"

# Saves every 8k steps
torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  --model.type "one-stage+7b" \
  --model.model_id "prism-clip+7b-webvid-train-80k-total-input-frames=16-videos-per-cluster=8-epochs=1-gpus=2-seed-1" \
  --seed 1 \
  --dataset.type "webvid" \
