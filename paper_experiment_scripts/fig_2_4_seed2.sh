...
# 40k total videos, 20k steps (bs = 2)

echo "Run fig_2_4_seed2"

torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  --model.type "one-stage+7b" \
  --model.model_id "prism-clip+7b-webvid-train-40k-total-input-frames=16-videos-per-cluster=4-epochs=1-gpus=2-seed2" \
  --dataset.type "webvid" \
  ...
  --model.ckpt_interval 20000 \
  --model.finetune_epochs 1 \
  --dataset.finetune_stage_components '["/vision/u/zanedurante/prismatic-vlms/dataset_splits/webvid_160k_video_split.json_video_40000_frames_4.json", ""]' \
