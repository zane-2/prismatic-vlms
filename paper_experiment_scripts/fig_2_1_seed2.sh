...
echo "Run fig_2_1_seed2"

torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  --model.type "one-stage+7b" \
  --model.model_id "prism-clip+7b-webvid-train-10k-total-input-frames=16-videos-per-cluster=1-epochs=1-gpus=2-seed2" \
  --dataset.type "webvid" \
