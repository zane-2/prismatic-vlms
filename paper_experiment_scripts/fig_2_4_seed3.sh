...
echo "Run fig_2_4_seed3"

torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  --model.type "one-stage+7b" \
  --model.model_id "prism-clip+7b-webvid-train-40k-total-input-frames=16-videos-per-cluster=4-epochs=1-gpus=2-seed3" \
  --dataset.type "webvid" \
