#!/bin/bash
#SBATCH --job-name=fig_1_16_seed3
...
echo "Running fig_1_16_seed3"

# Saves every 8k steps
torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  --model.type "one-stage+7b" \
  --model.model_id "prism-clip+7b-webvid-train-160k-total-input-frames=16-videos-per-cluster=16-epochs=1-gpus=2-seed3" \
  --seed 3 \
  --dataset.type "webvid" \
