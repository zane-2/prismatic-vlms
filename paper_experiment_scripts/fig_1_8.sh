...
torchrun --standalone --nnodes 1 --nproc-per-node 2 scripts/pretrain.py \
  ...
  --model.model_id "prism-clip+7b-webvid-train-160k-total-input-frames=16-videos-per-cluster=8-epochs=1-gpus=2" \
  --dataset.type "webvid" \
  ...
  --model.ckpt_interval 1000 \
  --model.finetune_epochs 1 \
  --dataset.finetune_stage_components '["/vision/u/zanedurante/prismatic-vlms/dataset_splits/webvid_160k_video_split.json_video_8_frames_2.json", ""]' \
