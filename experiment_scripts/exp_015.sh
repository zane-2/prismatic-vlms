...
torchrun --standalone --nnodes 1 --nproc-per-node 1 scripts/pretrain.py \
  ...
  --model.model_id "exp-015-prism-clip+7b-webvid-train-320k-videos-per-cluster=16-total-input-frames=16-epochs=5-gpus=1" \
  --dataset.type "webvid" \
  ...
  --dataset.type "webvid" \
  --dataset.finetune_stage_components '["/vision/u/silsingh/prismatic-vlms/dataset_splits/webvid_train_320k_videos_per_cluster=16_total_input_frames=16.json", "/vision/u/silsingh/prismatic-vlms/webvid_num_frames=1"]' \
